local config = function()
	...
	local Diagnostics = {
		...
		{
			provider = function(self)
				return self.errors > 0 and ("!" .. self.errors .. diagnostics_spacer) or ""
			end,
			hl = { fg = colors.samuraiRed },
		},
		{
			provider = function(self)
				return self.warnings > 0 and ("*" .. self.warnings .. diagnostics_spacer)
			end,
			hl = { fg = colors.roninYellow },
		},
		{
			provider = function(self)
				return self.info > 0 and ("I" .. self.info .. diagnostics_spacer)
			end,
			hl = { fg = colors.waveAqua1 },
		},
		{
			provider = function(self)
				return self.hints > 0 and ("?" .. self.hints .. diagnostics_spacer)
			end,
